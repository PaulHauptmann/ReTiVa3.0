...
def update_text_window(text_window, file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        #EmodbListe = [emo.emodbEmotionAnger , emo.emodbEmotionBoredom , emo.emodbEmotionDisgust , emo.emodbEmotionFear , emo.emodbEmotionHappiness , emo.emodbEmotionNeutral , emo.emodbEmotionSadness]
        
        #print(EmodbListe)
        
        
        #update_bar_chart(EmodbListe)
...
# Initialize the bar chart with all percentages at 0
bar_percentages = [0, 0, 0, 0, 0, 0, 0]
bar_heights = [percentage * 2.8 for percentage in bar_percentages]

# Create the bars on the bar chart
bars = [canvas.create_rectangle(25 + i * 35, 300, 50 + i * 35, 300 - height, fill="blue") for i, height in enumerate(bar_heights)]

# Update the bar chart with new percentages every 10 milliseconds
def update_bar_chart(percentage1, percentage2, percentage3, percentage4, percentage5, percentage6, percentage7):
    global bar_percentages
    bar_percentages = [percentage1, percentage2, percentage3, percentage4, percentage5, percentage6, percentage7]
    bar_heights = [percentage * 2.8 for percentage in bar_percentages]
    for i, bar in enumerate(bars):
        canvas.coords(bar, 25 + i * 35, 300, 50 + i * 35, 300 - bar_heights[i])
    window.after(10, update_bar_chart)

# Start updating the bar chart
update_bar_chart(0, 0, 0, 0, 0, 0, 0)
